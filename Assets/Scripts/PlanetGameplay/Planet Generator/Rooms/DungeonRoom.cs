...
public class DungeonRoom
{
	...
	private DirectionalArray<LockKeyType> exitLockType = new DirectionalArray<LockKeyType>();
	...
	public event Action<Direction, LockKeyType> OnExitLocked;
	...
	public void AddExit(Direction direction, IntPair exitPos)
	{
		...
		for (int i = 0; i < EXIT_WIDTH; i++)
		{
			...
			DungeonRoomObject newTrigger = new DungeonRoomObject(this, pos, "ExitTrigger", direction);
	...
	public void Lock(Direction direction, LockKeyType type)
	{
		exitLocked[direction] = true;
		exitLockType[direction] = type;
		OnExitLocked?.Invoke(direction, type);
	}
	...
	protected void LockAllExceptPreviousRoom()
	{
		...
		for (int i = 0; i < directions.Length; i++)
		{
			...
			{
				Lock(directions[i], LockKeyType.None);
			}
		}
	}

	protected void UnlockAllExitsOfLockType(LockKeyType type)
	{
		Direction[] directions = (Direction[])Enum.GetValues(typeof(Direction));
		for (int i = 0; i < directions.Length; i++)
		{
			Direction direction = directions[i];
			if (exitLockType[direction] != type) continue;
			Unlock(direction);
		}
	}

	protected void UnlockAllExitsOfLockTypeNone()
		=> UnlockAllExitsOfLockType(LockKeyType.None);

	public IntPair GetExitPos(Direction direction) => exitPositions[direction];

	public void AddKey(LockKeyType type)
	{
		IntPair pos = GetRandomPositionInRoom;
		DungeonRoomObject key = new DungeonRoomObject(this, pos, "Key", type);
		roomObjects.Add(key);
	}
